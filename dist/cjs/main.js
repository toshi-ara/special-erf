...
exports.erfc = exports.erf = void 0;
/**
 * ====================================================
...
const TINY = 1e-300;
const VERY_TINY = 2.848094538889218e-306; // 0x00800000, 0x00000000
// 2**-28 = 1/(1<<28) = 1/268435456
const SMALL = 3.725290298461914e-9;
// 2**-56 = 1/(2**56) = 1/72057594037927940
const SMALLerfc = 1.3877787807814457e-17;
const ERX = 8.45062911510467529297e-1; // 0x3FEB0AC1, 0x60000000
// Coefficients for approximation to erf on [0, 0.84375)
const EFX = 1.28379167095512586316e-1; // 0x3FC06EBA, 0x8214DB69
const EFX8 = 1.02703333676410069053; // 0x3FF06EBA, 0x8214DB69
const PP = [
    1.28379167095512558561e-1, // 0x3FC06EBA, 0x8214DB68
...
const PA = [
    -2.36211856075265944077e-3, // 0xBF6359B8, 0xBEF77538
...
const RA = [
    -9.86494403484714822705e-3, // 0xBF843412, 0x600D6435
...
const RB = [
    -9.86494292470009928597e-3, // 0xBF843412, 0x39E86F4A
...
function erf(x) {
    ...
    if (ax < 0.84375) {
        ...
        z = x * x;
        r = (0, helper_functions_1.polyval)(z, PP);
        s = (0, helper_functions_1.polyval)(z, QQ);
        y = r / s;
        return x + x * y;
    }
    // 0.84375 <= |x| < 1.25
    if (ax < 1.25) {
        s = ax - 1;
        p = (0, helper_functions_1.polyval)(s, PA);
        q = (0, helper_functions_1.polyval)(s, QA);
        if (sign) {
            return -ERX - p / q;
        }
        return ERX + p / q;
    ...
    // +inf > |x| >= 6
    if (ax >= 6) {
        return (sign) ? TINY - 1.0 : 1.0 - TINY; // raise inexact
    }
    s = 1.0 / (ax * ax);
    // |x| < 1/0.35 ~ 2.857143
    if (ax < 2.857142857142857) {
        r = (0, helper_functions_1.polyval)(s, RA);
        s = (0, helper_functions_1.polyval)(s, SA);
    }
    // |x| >= 1/0.35 ~ 2.857143
    else {
        r = (0, helper_functions_1.polyval)(s, RB);
        s = (0, helper_functions_1.polyval)(s, SB);
    }
    z = (0, helper_functions_1.setLowWord)(ax, 0); // pseudo-single (20-bit) precision x
    r = Math.exp(-z * z - 0.5625) * Math.exp((z - ax) * (z + ax) + r / s);
    return (sign) ? r / ax - 1 : 1 - r / ax;
}
exports.erf = erf;
// ERFC //
/**
